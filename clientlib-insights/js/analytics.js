(function (App) {
    ...
    setTimeout(function () {
      $input.daterangepicker(
        ...
        function (startMoment, endMoment) {
          ...
          // Call the API for the tab that is currently active
          fetchAnalyticsMetrics(isOrganicActive());
        }
  ...
  // Which tab is active? true => Organic, false => Overall
  function isOrganicActive() {
    const organicBtn = document.getElementById("organic-btn");
    return !!(organicBtn && organicBtn.classList.contains("active"));
  }

  // --- Build a cache key using mode + date range ---
  function getAnalyticsKey(isOrganic) {
    const mode = Boolean(isOrganic) ? 'organic' : 'overall';
    const s = currentRange ? moment(currentRange.start).format("YYYY-MM-DD") : 'NA';
    const e = currentRange ? moment(currentRange.end).format("YYYY-MM-DD") : 'NA';
    return `${mode}|${s}|${e}`;
  }
  ...
  function updatePageAnalyticsTiles(metrics = {}) {
    ...
    if (faqClickedEl) {
      ...
      const v = Number(faqData) || 0;
      faqClickedEl.textContent = Number.isFinite(v)
        ? v.toLocaleString()
        : faqData;
    }
  ...
  async function fetchAnalyticsMetrics(isOrganic) {

      // NEW: return early if we already have data for this (mode + date range)
    try {
      const key = getAnalyticsKey(isOrganic);
      const cached = App.state.analyticsCache[key];
      if (cached) {
        updatePageAnalyticsTiles(cached);
        renderMetricsBySection(
          cached,
          "userBehaviourMetricsContainer",
          SECTION_MAPPING.userBehaviour
        );
        App.state.lastRenderedKey = key;
        return; // ← IMPORTANT: skip network fetch
      }
    } catch (e) {
      // If anything goes wrong with keying, fall through to normal fetch
      console.warn("Analytics cache key error:", e);
    }
    ...
    try {
      ...
      if (currentRange) {
        ...
        //   url.searchParams.set("organicFlag" , isOrganic)
        // }
        url.searchParams.set("isOrganic", Boolean(isOrganic));
      ...
      // NEW: persist to cache for this (mode + date range)
      const keyJustFetched = getAnalyticsKey(isOrganic);
      App.state.analyticsCache[keyJustFetched] = metrics;
      App.state.lastRenderedKey = keyJustFetched;
