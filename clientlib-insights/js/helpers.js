...
(function (App) {
  ...
  function getCurrentPageUrlForApis() {
    ...
    // Create the base clean URL (stops at .com or .html)
    let cleanedHref = `${urlObj.protocol}//${urlObj.host}${urlObj.pathname}`;

    const href = cleanedHref;
    if (
    ...
    } else if (href.includes("https://beta")) {
      return href.replace("https://beta", "https://www");
    }
    return href;
